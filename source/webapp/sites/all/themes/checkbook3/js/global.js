...
// Added by Momix
function transformLine( lne ) {
    if (jQuery('#' + lne).is(':visible')) {
        jQuery('#' + lne).fadeOut();
    } else {
        jQuery('#' + lne).fadeIn();
    }
...
"#agency-list",
"#block-block-8",
"#block-checkbook-smart-search-checkbook-smart-search-form",
"#block-block-7"
];
    index = jQuery.inArray(filename, boringPages);
    
    if (index > -1) {
        for (index = 0; index < unwanteds.length; ++index) {
            jQuery(unwanteds[index]).hide();
        }
    }
}